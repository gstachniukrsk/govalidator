...
func TestIntValidator(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		...
		for i, v := range ds {
			twig, errs := validator.IntValidator(nil, v)
	...
	t.Run("not a int - string", func(t *testing.T) {
		twig, errs := validator.IntValidator(nil, "1")
		assert.True(t, twig)
		assert.Equal(t, []error{validator.NotAnIntegerError{}}, errs)
	})

	t.Run("not a int - float with precision", func(t *testing.T) {
		var f interface{}
		f = 1.1
		twig, errs := validator.IntValidator(nil, f)
		assert.True(t, twig)
		assert.Equal(t, []error{validator.NotAnIntegerError{}}, errs)
	})
